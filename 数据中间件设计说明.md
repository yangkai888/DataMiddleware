...
- **运行环境**：
  - 服务部署在 **Linux 操作系统**（推荐 CentOS 7+、Ubuntu 18.04+ 或其他主流 Linux 发行版）。
  - 支持 **x86_64** 和 **ARM64** 架构。
  - 使用 **systemd** 进行服务管理，或通过 **Docker 容器** 部署。
...
- **运行环境**：
  ...
  - **部署方式**：支持 systemd 服务管理、Docker 容器部署、Kubernetes 编排；
  - **架构支持**：x86_64（主要）、ARM64（可选）。

- **语言与运行时**：
  - 使用 **Go** 语言，利用其 `goroutine + channel` 并发模型，简化高并发 IO 服务开发；
  - Go 编译为静态二进制文件，**无需在 Linux 服务器上安装额外运行时依赖**。
...
4. **Linux 部署规范**：
   - **服务管理**：使用 systemd 管理服务（`/etc/systemd/system/game-middleware-{game_id}.service`）；
   - **二进制文件**：存放于 `/opt/game-middleware/{game_id}/bin/`；
   - **配置文件**：存放于 `/opt/game-middleware/{game_id}/conf/` 或 `/etc/game-middleware/{game_id}/`；
   - **日志目录**：`/var/log/game-middleware/{game_id}/`（建议配合 logrotate 日志轮转）；
   - **运行用户**：建议创建专用用户（如 `gameapi`），避免使用 root 运行服务；
   - **端口规划**：每个游戏使用独立端口段（如游戏A: 8001-8003，游戏B: 8011-8013）。
   - **systemd 服务配置示例**（`/etc/systemd/system/game-middleware-game_a.service`）：
     ```ini
     [Unit]
     Description=Game Middleware Service - Game A
     ...
     User=gameapi
     Group=gameapi
     WorkingDirectory=/opt/game-middleware/game_a
     ExecStart=/opt/game-middleware/game_a/bin/middleware -config /opt/game-middleware/game_a/conf/config.yaml
     Restart=always
     RestartSec=5
     LimitNOFILE=65535
   ...
   - **常用管理命令**：
     ```bash
     # 启动/停止/重启服务
     systemctl start game-middleware-game_a
     systemctl stop game-middleware-game_a
     systemctl restart game-middleware-game_a
     
     # 查看服务状态和日志
     systemctl status game-middleware-game_a
     journalctl -u game-middleware-game_a -f
     ```
...
- **日志实现建议（Go）**：
  ...
  - **Linux 日志管理**：
    - 日志文件路径：`/var/log/game-middleware/{game_id}/app.log`；
    - 配合 **logrotate** 进行日志轮转，避免磁盘空间耗尽；
    - 示例 logrotate 配置（`/etc/logrotate.d/game-middleware`）：
      ```
      /var/log/game-middleware/*/app.log {
          daily
          rotate 30
          compress
          delaycompress
